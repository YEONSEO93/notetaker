...
const BottomTab = () => {
  ...
  const goToScreen = (screenName) => {
    if (!isAuthenticated && screenName === "AccountDetails") {
      setShowLogin(true);
    } else {
      navigation.navigate(screenName);
    }
  ...
  return (
    <>
      <Container theme={theme}>
        ...
        <IconContainer onPress={() => goToScreen("Calendar")} theme={theme} iconSize={iconSize}>
          <Ionicons name="calendar-outline" size={iconSize} color={theme.textColor} />
          <TabLabel theme={theme} textSize={textSize}>Calendar</TabLabel>
        </IconContainer>
        <IconContainer onPress={() => goToScreen("Setting")} theme={theme} iconSize={iconSize}>
          <Ionicons name="settings-outline" size={iconSize} color={theme.textColor} />
          <TabLabel theme={theme} textSize={textSize}>Setting</TabLabel>
        </IconContainer>
      </Container>
      {showLogin && <LoginScreen onSuccess={handleLoginSuccess} />}
